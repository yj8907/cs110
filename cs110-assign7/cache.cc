...
bool HTTPCache::containsCacheEntry(const HTTPRequest &request, HTTPResponse &response) const {
    ...
    if (!cachedEntryIsValid(cachedFileName)) { // if it's not valid, then remove it
        ...
        if (rmdir(fullCacheDirectoryName.c_str()) !=
            0) // rmdir succeeds provided the supplied directory is empty (remove would have works as well)
    ...
    try {
        ...
        cerr << oslock << "     [Problem rehydrating previously cached response for " << request.getURL() << ".]"
             << endl;
...
void HTTPCache::ensureDirectoryExists(const string &directory, bool empty) const {
    ...
    if (!startsWith(directory, cacheDirectory))
        throw HTTPCacheConfigException(
                "You are nuts for calling this function on a directory not rooted in your proxy cache.");
    ...
    while (true) {
        ...
        if (lstat(dirEntry.c_str(), &st) != 0)
            throw HTTPCacheConfigException(
                    string("Cache is malformed... manually delete ") + dirEntry + " " + strerror(errno));
        if (S_ISDIR(st.st_mode))
...
void
HTTPCache::extractCreateAndExpireTimes(const string &cachedFileName, time_t &createTime, time_t &expirationTime) const {
    if (!cacheEntryFileNameIsProperlyStructured(cachedFileName)) {
        throw HTTPCacheAccessException(
                cachedFileName + " doesn't confirm to the naming convention we expect... clear the cache.");
    }
