...
void RSSFeedList::parse() throw(RSSFeedListException) {
    ...
    for (int i = 0; i < numItems; i++) {
        ...
        xmlChar *title =
                (titles->nodesetval == NULL || titles->nodesetval->nodeNr == 0) ? xmlCharStrdup("") : xmlNodeGetContent(
                        titles->nodesetval->nodeTab[0]);
        ...
        xmlChar *link =
                (links->nodesetval == NULL || links->nodesetval->nodeNr == 0) ? xmlCharStrdup("") : xmlNodeGetContent(
                        links->nodesetval->nodeTab[0]);
