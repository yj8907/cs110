...
static bool articleCmp(const Article &a, const Article &b) {
    if (a.title < b.title) return true;
    if (a.title > b.title) return false;
    if (getURLServer(a.url) < getURLServer(b.url)) return true;
    return false;
}
 ...
 */
    ...
    /**
 * Method: aggregateArticles
 * -----------------------
 * Aggregate articles according to title and server names
 */
    void aggregateArticles(std::vector<Article> &articles);
