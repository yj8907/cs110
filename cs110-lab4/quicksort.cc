...
static void conservativeQuicksort(vector<int> &numbers, ssize_t start, ssize_t finish) {
    ...
    thread childThreads[1];
    childThreads[0] = thread(conservativeQuicksort, ref(numbers), start, mid -1 );
    conservativeQuicksort(numbers, mid + 1, finish);
    for (auto &t : childThreads) t.join();
...
static void aggressiveQuicksort(vector<int> &numbers, ssize_t start, ssize_t finish) {
    ...
    if (start >= finish) return;
    ssize_t mid = partition(numbers, start, finish);
    thread childThreads[2];
    childThreads[0] = thread(aggressiveQuicksort, ref(numbers), start, mid -1 );
    childThreads[1] = thread(aggressiveQuicksort, ref(numbers), mid + 1, finish);
...
static void aggressiveQuicksort(vector<int> &numbers) {
    ...
    ssize_t start = 0;
    ssize_t finish = numbers.size() - 1;
    aggressiveQuicksort(numbers, start, finish);
}
...
int main(int argc, char *argv[]) {
    ...
    for (size_t trial = 1; trial <= 1000; trial++) {
        ...
        if (!sorted) {
            cout << endl << "quicksort is \033[1;31mBROKEN.... please fix!\033[0m" << flush;
            break;
        }
    }
    cout << endl;
